    ...
    for vendor in [FUSION_VENDOR, MELLANOX_VENDOR]:
        ...
        for check_dev in check_devs:
            ...
            for dev in dl.walk_from_root(check_dev):
                ...
                if maxreadreq != bytes_size.index(MAX_READREQ):
                    print "%s  Setting MaxReadReq to %d" % (indent, MAX_READREQ)
                    dev.config.write("pcie_device_control_max_read_request_size", bytes_size.index(MAX_READREQ))
                dev_index += 1
                print
            print
